...
// only in this translation unit, used by list commands only
static SandstoneTestSet *test_set;

auto collate_test_groups() {
    struct Group {
...
void list_tests(bool include_tests, bool include_groups, bool include_descriptions, const SandstoneApplication* app)
{
...
void list_group_members(const char *groupname)
{
    auto groups = collate_test_groups();
    for (auto pair : groups) {
        const auto &g = pair.second;
...
std::unique_ptr<ParsedOpts> parse_and_validate(int argc, char **argv, SandstoneApplication* app) {
    std::unique_ptr<ParsedOpts> res = std::make_unique<ParsedOpts>();
    ...
    desc.add_options()
        ...
        ("dump-cpu-info", po::value<bool>()->implicit_value(true)->zero_tokens(),
            "Prints the CPU information that the tool detects (package ID, core ID, " \
            "thread ID, microcode, and PPIN) then exit.")
        ("on-hang", po::value<std::string>(&res->on_hang_arg), "On hang")
        ("on-crash", po::value<std::string>(&res->on_crash_arg), "On crash")
        ("ignore-os-errors", po::bool_switch(&app->ignore_os_errors),
            "Continue execution of Sandstone even if a test encounters an operating " \
            "system error")
        ("ignore-timeout", po::bool_switch(&res->test_set_config.ignore_unknown_tests),
            "Continue execution of Sandstone even if a test times out.")
        ("threads,n", po::value<int>(),
            "Set the number of threads to be run to <NUMBER>. If not specified or if " \
            "0 is passed, then the test defaults to the number of CPUs in the system. " \
            "Note the --cpuset and this parameter do not behave well together.")
        ...
        ("is-debug-build", po::value<bool>(), "Is debug build")
#endif
#if defined(__SANITIZE_ADDRESS__)
        ("is-asan-build", po::value<bool>(), "Is ASAN build")
#endif
    ...
    if (!SandstoneConfig::RestrictedCommandLine) {
        ...
        if (vm.count("list") || vm.count("list-tests") || vm.count("list-groups")) {
            ...
            test_set = new SandstoneTestSet(res->test_set_config, SandstoneTestSet::enable_all_tests);
            list_tests(!vm.count("list-groups"), !vm.count("list-tests"), vm.count("list"), app);
            return nullptr;
        }
        ...
        if (vm.count("timeout")) {
            app->max_test_time = string_to_millisecs(vm["timeout"].as<std::string>());
        }
        if (vm.count("threads")) {
        ...
        if (vm.count("use-builtin-test-list")) {
            ...
            res->builtin_test_list_name = value ? value : "auto";
        }
        ...
        if (vm.count("test-delay")) {
            app->delay_between_tests = string_to_millisecs(vm["test-delay"].as<char*>());
        }
        ...
        if (vm.count("test-range")) {
            if (parse_testrun_range(vm["test-range"].as<char*>(), res->starting_test_number, res->ending_test_number) == EXIT_FAILURE) {
