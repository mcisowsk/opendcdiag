...
void list_tests(SandstoneTestSet& test_set, int opt, SandstoneApplication* app)
{
...
struct CmdlineParser {
    struct ArgsPresent {
        ...
        std::optional<const char*> gdb_server_comm;
        ...
        while ((opt = simple_getopt(argc, argv, long_options, &coptind)) != -1) {
            ...
            case cpuset_option:
                if (args_present.cpuset) {
                    throw std::invalid_argument{"cpuset defined more than once"};
                }
    ...
    int validate_args() const
    {
        if ((int)args_present.one_sec + (int)args_present.thirty_sec + (int)args_present.two_min + (int)args_present.five_min + (int)args_present.total_time.has_value() >= 2) {
            throw std::invalid_argument{"Options 1sec, 30sec, 2min, 5min, total-time are mutually exclusive"};
        }
        ...
        if ((int)args_present.list_tests + (int)args_present.list_groups + (int)args_present.list_group_members + (int)args_present.dump_cpu_info + (int)args_present.print_version >= 2) {
            throw std::invalid_argument{"Options list-tests, list-groups, list-group-members, dump-cpu-info, version are mutually exclusive"};
        }

        if ((int)args_present.alpha + (int)args_present.beta + (int)args_present.quality.has_value() + (int)args_present.selftest >= 2) {
            throw std::invalid_argument{"Options alpha, beta, quality, selftests are mutually exclusive"};
        }

        if (args_present.max_cores_per_slice && args_present.no_slicing) {
            throw std::invalid_argument{"Options no-slicing, max-cores-per-slice are mutually exclusive"};
        }
        ...
        if (args_present.output_format && (args_present.output_yaml_format || args_present.output_yaml_indent)) {
            if (strcmp(*args_present.output_format, "yaml") != 0) {
                throw std::invalid_argument{"Options yaml, output-format are mutually exclusive"};
            }
        }

        if (args_present.quiet && verbosity > -1) {
            throw std::invalid_argument{"Options quiet, verbose are mutually exclusive"};
        }

        if (args_present.total_time && args_present.service) {
            throw std::invalid_argument{"Options total-time, service are mutually exclusive"};
        }
    ...
    std::optional<int> parse_args(SandstoneApplication* app, ParsedCmdLineOpts& opts, char** argv)
    {
        ...
        if (args_present.cpuset) {
            apply_cpuset_param(app, &(*args_present.cpuset)[0]);
        }
        ...
        if (args_present.list_tests) {
            ...
            list_tests(test_set, raw_list_tests, app);
            return EXIT_SUCCESS;
        }
        if (args_present.list_groups) {
            SandstoneTestSet test_set{opts.test_set_config, SandstoneTestSet::enable_all_tests};
            list_tests(test_set, raw_list_groups, app);
            return EXIT_SUCCESS;
        }
        ...
        if (args_present.dump_cpu_info) {
            dump_cpu_info(app->shmem->verbosity);
            return EXIT_SUCCESS;
        }
        ...
        if (args_present.fork_mode) {
            ...
            if (strcmp(value, "no") == 0 || strcmp(value, "no-fork") == 0) {
                ...
                throw std::invalid_argument("unknown value to -f");
            }
        }
    ...
    std::optional<int> handle_restricted_command_line(int argc, char** argv, SandstoneApplication* app, ParsedCmdLineOpts& opts) {
...
std::optional<int> parse_cmdline(int argc, char** argv, SandstoneApplication* app, ParsedCmdLineOpts& opts) {
    // isolate CmdlineParser existance so that we create it only for the sake of cmdline parsing
    CmdlineParser parser;
    if (SandstoneConfig::RestrictedCommandLine) {
        return parser.handle_restricted_command_line(argc, argv, app, opts);
    }
