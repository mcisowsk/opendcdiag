...
namespace {
std::unique_ptr<ParsedOpts> call_parse_and_validate(const char* cmdline, SandstoneApplication* app) {
...
sApp_ptr mock_sApp() {
    ...
    // we do not set shmemfd, shmem->thread_data_offset, shmem->main_process_pid, just zero inirialize SharedMemory
    return res;
}

auto pred3 = [](const char* what, const char* str1, const char* str2) {
...
TEST(OptsValidation, Verbosity) {
    ...
    try {
        auto opts = call_parse_and_validate(cmdline, app.get());
